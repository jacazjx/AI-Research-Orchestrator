...
def resolve_semantic_scholar_key(args: argparse.Namespace) -> tuple[str, str, str]:
    ...
    if args.no_key_prompt or not sys.stdin.isatty():
        print(
            "No Semantic Scholar API key found. Continuing with the free shared mode.",
            file=sys.stderr,
        )
        return "", "", "shared"
    ...
    while True:
        ...
        try:
            ...
            if retryable and (retry_forever or attempt < max_retries):
                attempt += 1
                if PROGRESS_TRACKER is not None:
                    PROGRESS_TRACKER.bump_retry(
                        detail=f"{progress_label} retry in {backoff}s after HTTP {exc.code}"
                    )
                time.sleep(backoff)
            ...
            if retry_forever or attempt < max_retries:
                ...
                if PROGRESS_TRACKER is not None:
                    PROGRESS_TRACKER.bump_retry(
                        detail=f"{progress_label} retry in {backoff}s after network error"
                    )
                time.sleep(backoff)
                backoff = min(max_backoff, max(backoff + 1, int(backoff * 1.8)))
                continue
...
def semantic_scholar_search(
    ...
    *,
    ...
    params = urlencode(
        {"query": search_query, "limit": str(limit), "fields": SEMANTIC_SCHOLAR_FIELDS}
    )
        ...
        try:
            ...
            if PROGRESS_TRACKER is not None:
                PROGRESS_TRACKER.update(
                    stage="Falling back to shared search",
                    detail="Authenticated Semantic Scholar access was refused.",
                    mode="shared",
                )
...
def openalex_lookup_doi(doi: str, *, user_agent: str, mailto: str | None) -> dict[str, Any] | None:
    ...
    url = (
        f"https://api.openalex.org/works/https://doi.org/{quote(normalized, safe='')}{query_suffix}"
    )
    payload = http_get_json(
        url,
...
def openalex_search_query(
    query: str, *, limit: int, user_agent: str, mailto: str | None
) -> list[dict[str, Any]]:
    params: list[tuple[str, str]] = [
...
def crossref_search_title(
    title: str, *, limit: int, user_agent: str, mailto: str | None
) -> list[dict[str, Any]]:
...
def infer_preprint(
    venue: str, url: str, publication_types: list[str], external_ids: dict[str, Any]
) -> bool:
...
def candidate_passes_relevance_gate(
    candidate: dict[str, Any], *, min_relevance_score: float
) -> bool:
...
def parse_local_bib_entry(entry: str, bib_path: Path) -> dict[str, Any]:
    ...
    manual_check_required = bibtex_field_value(entry, "x-secondary-check-required").casefold() in {
        "1",
        "true",
        "yes",
    }
    ...
    preprint = infer_preprint(
        venue, url, [], {"ArXiv": bool(re.search(r"arxiv", url, flags=re.IGNORECASE))}
    )
...
def update_ledger_entry(
    ...
    *,
    ...
    merged["manualCheckRequired"] = bool(
        record.get("manualCheckRequired") or merged.get("manualCheckRequired")
    )
    merged["checked"] = bool(
        infer_local_checked_status(record) if checked_override is None else checked_override
...
def sync_local_bib_entries(
    ...
    for bib_path in bib_paths:
        ...
        file_entries = [
            parse_local_bib_entry(entry, resolved) for entry in split_bibtex_entries(text)
        ]
...
def search_local_bib_candidates(
    ...
    *,
    ...
    for entry in local_entries:
        ...
        candidate["evidenceScore"] = infer_evidence_score(
            title, "", bool(candidate.get("peerReviewed", False))
        )
...
def enrich_with_verified_metadata(
    ...
    *,
    ...
    if doi:
        ...
        try:
            openalex_payload = openalex_lookup_doi(
                doi, user_agent=user_agent, mailto=crossref_mailto
            )
            if openalex_payload:
    ...
    if openalex_limit > 0 and (
        not openalex_meta or result.get("preprint") or not openalex_meta.get("doi")
    ):
        try:
    ...
    if crossref_meta:
        if not replace_with_formal_version(
            result, crossref_meta, source_name="crossref", notes=notes
        ):
    ...
    if openalex_meta:
        ...
        merge_missing_fields(
            result, openalex_meta, ["doi", "year", "venue", "abstract", "url", "citationCount"]
        )
    ...
    if dblp_limit > 0:
        try:
            dblp_candidates = dblp_search_title(
                result["title"], limit=dblp_limit, user_agent=user_agent
            )
    ...
    result["relevanceScore"] = infer_relevance_score(
        query, result["title"], str(result.get("abstract", ""))
    )
    result["evidenceScore"] = infer_evidence_score(
        result["title"],
        ...
        try:
            ...
            if not result.get("doi"):
                manual_check_reasons.append(
                    "formal-version-has-no-doi; bibtex-downloaded-from-dblp"
                )
    ...
    result["googleScholarSearchUrl"] = google_scholar_search_url(
...
def load_venue_hints(path: str | None) -> dict[str, Any]:
    ...
    if not isinstance(venues, dict):
        raise ValueError(
            "Venue hints must be a JSON object or contain a top-level 'venues' object."
        )
    ...
    for item in report:
        ...
        for index, result in enumerate(results, start=1):
            ...
            if result.get("manualCheckRequired"):
                reason_text = (
                    "; ".join(str(reason) for reason in result.get("manualCheckReasons", []))
                    or "manual second check required"
                )
...
def main() -> int:
    ...
    local_reference_entries = merge_candidates(
        local_reference_entries, user_library_search_entries(user_library)
    )
    ...
    for query_index, query in enumerate(queries, start=1):
        local_candidates = search_local_bib_candidates(
            query, local_reference_entries, limit=args.local_bib_limit
        )
        PROGRESS_TRACKER.update(
        ...
        semantic_candidates = [
            semantic_scholar_record(raw_candidate, query) for raw_candidate in raw_candidates
        ]
        PROGRESS_TRACKER.update(
            current_query=query_index,
        ...
        if args.openalex_limit > 0:
            try:
                ...
                openalex_candidates = [
                    candidate
                    for candidate in (
                        openalex_record(item, query) for item in openalex_raw_candidates
                    )
                    if candidate_passes_relevance_gate(
                        candidate, min_relevance_score=args.min_relevance_score
                    )
                ]
            except FetchError as exc:
                openalex_candidates = []
